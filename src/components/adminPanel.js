...
export function renderAdminPanel(container, missions, rules, onUpdate, onLogout) {
      ...
      <div class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex gap-1">
            <button type="button" id="tab-missions" class="tab-btn px-5 py-3 font-semibold border-b-2 transition-colors ${currentTab === 'missions' ? 'border-gray-800 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700'}">
              <span class="material-icons-outlined text-lg align-middle mr-1">assignment</span>
              미션 관리
            </button>
            <button type="button" id="tab-rules" class="tab-btn px-5 py-3 font-semibold border-b-2 transition-colors ${currentTab === 'rules' ? 'border-gray-800 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700'}">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div id="missions-tab" class="${currentTab === 'missions' ? '' : 'hidden'}">
            <div class="flex justify-end mb-4">
              <button type="button" id="add-mission-btn" class="admin-btn-add">
                <span class="material-icons-outlined text-base">add</span>
                미션 추가
              </button>
            </div>
            ${renderMissionForm()}
            <div class="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-4" id="missions-list">
          ...
          <div id="rules-tab" class="${currentTab === 'rules' ? '' : 'hidden'}">
            <div class="flex justify-end mb-4">
              <button type="button" id="add-rule-btn" class="admin-btn-add">
                <span class="material-icons-outlined text-base">add</span>
                규칙 추가
              </button>
            </div>
            ${renderRuleForm()}
            <div class="space-y-3 mt-4" id="rules-list">
  ...
  document.getElementById('tab-missions').addEventListener('click', () => {
    ...
    document.getElementById('tab-missions').classList.add('border-gray-800', 'text-gray-900')
    document.getElementById('tab-missions').classList.remove('border-transparent', 'text-gray-500')
    document.getElementById('tab-rules').classList.remove('border-gray-800', 'text-gray-900')
  ...
  document.getElementById('tab-rules').addEventListener('click', () => {
    ...
    document.getElementById('tab-rules').classList.add('border-gray-800', 'text-gray-900')
    document.getElementById('tab-rules').classList.remove('border-transparent', 'text-gray-500')
    document.getElementById('tab-missions').classList.remove('border-gray-800', 'text-gray-900')
...
function renderMissionForm(isEditing = false, mission = null) {
  ...
  return `
    <div id="mission-form-container" class="admin-form-card p-5 sm:p-6 ${isEditing ? '' : 'hidden'}">
      <div class="flex items-center gap-3 mb-6">
        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center">
          <span class="material-icons-outlined text-gray-600 text-xl">${isEditing ? 'edit' : 'add_circle'}</span>
        </div>
        <h5 class="text-lg sm:text-xl font-bold text-gray-800">${isEditing ? '미션 수정' : '새 미션 추가'}</h5>
      </div>
      <form id="mission-form" class="space-y-5">
        ...
        <div>
          <label for="title" class="block mb-2 text-sm font-semibold text-gray-700">제목 *</label>
          <input
            type="text"
            id="title"
            name="title"
            ...
            class="bg-white border border-gray-300 text-gray-800 text-sm rounded-lg focus:ring-1 focus:ring-gray-400 focus:border-gray-400 block w-full p-2.5 transition-colors"
            placeholder="미션 제목을 입력하세요"
          >
        </div>

        <div>
          <label for="icon" class="block mb-2 text-sm font-semibold text-gray-700">아이콘</label>
          <div class="flex items-center gap-3">
            <div id="icon-preview" class="w-12 h-12 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center">
              <span class="material-icons-outlined text-2xl text-gray-600">${selectedIcon}</span>
            </div>
            <select
              id="icon"
              name="icon"
              class="flex-1 bg-white border border-gray-300 text-gray-800 text-sm rounded-lg focus:ring-1 focus:ring-gray-400 focus:border-gray-400 block p-2.5 transition-colors"
            >
              ${iconOptions}
            </select>
          </div>
        ...
        <div>
          <label for="description" class="block mb-2 text-sm font-semibold text-gray-700">설명</label>
          <textarea
            id="description"
            name="description"
            rows="3"
            class="block p-2.5 w-full text-sm text-gray-800 bg-white rounded-lg border border-gray-300 focus:ring-1 focus:ring-gray-400 focus:border-gray-400 transition-colors"
            placeholder="미션 설명을 입력하세요"
          >${escapeHtml(mission?.description || '')}</textarea>
        </div>

        <div>
          <label for="condition" class="block mb-2 text-sm font-semibold text-gray-700">성공 조건</label>
          <input
            type="text"
            id="condition"
            name="condition"
        ...
        <div class="flex gap-2 pt-2">
          ...
          <button type="button" id="cancel-form-btn" class="admin-btn-secondary">
    ...
    <article class="admin-card p-4 sm:p-5">
      ...
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
          <span class="material-icons-outlined text-gray-500 text-xl">${mission.icon || 'assignment'}</span>
        </div>
      ...
      <!-- 성공조건 -->
      <div class="admin-condition ${mission.condition ? '' : 'opacity-50'}">
      ...
      <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
        ...
        <div class="flex items-center gap-2 mr-auto">
          <span class="text-sm font-medium text-gray-400 w-6 text-center">${index + 1}</span>
          <button
            class="move-up-btn w-7 h-7 flex items-center justify-center rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            data-id="${mission.id}"
            ${isFirst ? 'disabled' : ''}
            title="위로 이동"
          >
          ...
          <button
            class="move-down-btn w-7 h-7 flex items-center justify-center rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            data-id="${mission.id}"
            ${isLast ? 'disabled' : ''}
            title="아래로 이동"
          >
        ...
        <!-- 수정/삭제 -->
        <button
          class="edit-mission-btn px-3 py-1.5 text-xs font-medium rounded transition-colors border border-gray-300 text-gray-600 hover:bg-gray-50"
          data-id="${mission.id}"
        >
          수정
        </button>
        <button
          class="delete-mission-btn px-3 py-1.5 text-xs font-medium rounded transition-colors border border-red-200 text-red-500 hover:bg-red-50"
          data-id="${mission.id}"
        >
          삭제
        </button>
...
function renderEmptyState() {
  return `
    <div class="text-center py-10">
      <span class="material-icons-outlined text-4xl text-gray-300 mb-3">assignment</span>
      <h3 class="text-base font-medium text-gray-600 mb-1">아직 미션이 없습니다</h3>
      <p class="text-sm text-gray-400">새 미션을 추가해보세요</p>
    </div>
  `
}
...
function renderRuleForm(isEditing = false, rule = null) {
  return `
    <div id="rule-form-container" class="admin-form-card p-5 sm:p-6 ${isEditing ? '' : 'hidden'}">
      <div class="flex items-center gap-3 mb-6">
        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center">
          <span class="material-icons-outlined text-gray-600 text-xl">${isEditing ? 'edit' : 'add_circle'}</span>
        </div>
        <h5 class="text-lg sm:text-xl font-bold text-gray-800">${isEditing ? '규칙 수정' : '새 규칙 추가'}</h5>
      </div>
      <form id="rule-form" class="space-y-4">
        ...
        <div>
          <label for="rule-content" class="block mb-2 text-sm font-semibold text-gray-700">규칙 내용 *</label>
          <textarea
            id="rule-content"
            name="content"
            required
          ...
          <p class="mt-2 text-xs text-gray-500">**텍스트** 형식으로 작성하면 굵게 표시됩니다.</p>
        </div>
        ...
        <div class="flex gap-2 pt-2">
          ...
          <button type="button" id="cancel-rule-btn" class="admin-btn-secondary">
            <span class="material-icons-outlined text-base">close</span>
            취소
          </button>
        </div>
    ...
    <article class="admin-card p-4 sm:p-5">
      ...
      <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
        ...
        <div class="flex items-center gap-2 mr-auto">
          <span class="text-sm font-medium text-gray-400 w-6 text-center">${index + 1}</span>
          <button
            class="move-rule-up-btn w-7 h-7 flex items-center justify-center rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            data-id="${rule.id}"
            ${isFirst ? 'disabled' : ''}
            title="위로 이동"
          >
          ...
          <button
            class="move-rule-down-btn w-7 h-7 flex items-center justify-center rounded border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            data-id="${rule.id}"
            ${isLast ? 'disabled' : ''}
            title="아래로 이동"
          >
        ...
        <!-- 수정/삭제 -->
        <button
          class="edit-rule-btn px-3 py-1.5 text-xs font-medium rounded transition-colors border border-gray-300 text-gray-600 hover:bg-gray-50"
          data-id="${rule.id}"
        >
          수정
        </button>
        <button
          class="delete-rule-btn px-3 py-1.5 text-xs font-medium rounded transition-colors border border-red-200 text-red-500 hover:bg-red-50"
          data-id="${rule.id}"
        >
          삭제
        </button>
...
function renderEmptyRulesState() {
  return `
    <div class="text-center py-10">
      <span class="material-icons-outlined text-4xl text-gray-300 mb-3">rule</span>
      <h3 class="text-base font-medium text-gray-600 mb-1">등록된 규칙이 없습니다</h3>
      <p class="text-sm text-gray-400">새 규칙을 추가해보세요</p>
    </div>
  `
}
